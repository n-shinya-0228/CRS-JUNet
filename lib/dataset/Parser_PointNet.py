# Parser for the Step C handcrafted + PointNet hybrid input.
import os
import torch
from torch.utils.data import DataLoader

from .SemanticKitti_PointNet import SemanticKitti

def bev_collate_fn(batch):
    """
    SemanticKitti_PointNet が返す16要素のタプルをバッチ化する関数。
    テンソル化すべき最初の3つ（特徴量、マスク、ラベル）だけスタックし、
    残りのダミーやパス情報はリストのまま返す。
    """
    # 7ch handcrafted + 16ch PointNet = 23ch. Trainer adds the mask channel.
...
class Parser:
    ...
    def __init__(self, root, data_cfg, arch_cfg, gt=True, shuffle_train=True):
        ...
        pointnet_cfg = arch_cfg["dataset"].get("pointnet", {})
        self.pointnet_folder = pointnet_cfg.get("folder", "pointnet_16ch")
        self.pointnet_feature_dim = int(pointnet_cfg.get("feature_dim", 16))
        self.pointnet_max_radius = float(pointnet_cfg.get("max_radius", 51.2))
        ...
        self.train_dataset = SemanticKitti(
            ...
            gt=self.gt, is_train=True, copy_paste_prob=self.copy_paste_prob,
            pointnet_folder=self.pointnet_folder,
            pointnet_feature_dim=self.pointnet_feature_dim,
            pointnet_max_radius=self.pointnet_max_radius,
        )
