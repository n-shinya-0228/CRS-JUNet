...
class SemanticKitti(Dataset):
    ...
    def __init__(self, root, sequences, labels, color_map,
                 ...
                 pointnet_folder="pointnet_16ch", pointnet_feature_dim=16,
                 pointnet_max_radius=51.2):
        super().__init__()
        ...
        self.pointnet_folder = str(pointnet_folder)
        self.pointnet_feature_dim = int(pointnet_feature_dim)
        self.pointnet_max_radius = float(pointnet_max_radius)
        self.height = int(sensor["img_prop"]["height"])
        self.width = int(sensor["img_prop"]["width"])

        if self.pointnet_feature_dim <= 0:
            raise ValueError("pointnet_feature_dim must be positive")
        ...
        self._validate_pointnet_coverage()
    ...
    def get_pointnet_file(self, bev_file):
        seq_dir = os.path.dirname(os.path.dirname(bev_file))
        return os.path.join(seq_dir, self.pointnet_folder, os.path.basename(bev_file))

    def _validate_pointnet_coverage(self):
        for bev_file in self.scan_files:
            pointnet_file = self.get_pointnet_file(bev_file)
            if not os.path.isfile(pointnet_file):
                raise FileNotFoundError(
                    f"Missing PointNet feature for {bev_file}: {pointnet_file}"
                )

    def load_pointnet_feature(self, bev_file):
        pointnet_file = self.get_pointnet_file(bev_file)
        with gzip.open(pointnet_file, "rb") as f:
            data = torch.load(f, map_location="cpu", weights_only=True)

        required = {"cell_ids", "features", "feature_dim", "H", "W", "max_radius"}
        missing = required.difference(data)
        if missing:
            raise KeyError(
                f"PointNet feature file {pointnet_file} is missing keys: {sorted(missing)}"
            )
        ...
        if (feature_dim, height, width) != (
            self.pointnet_feature_dim, self.height, self.width
        ):
            raise ValueError(
                f"PointNet metadata mismatch in {pointnet_file}: "
                f"got ({feature_dim}, {height}, {width}), expected "
                f"({self.pointnet_feature_dim}, {self.height}, {self.width})"
            )
        if abs(max_radius - self.pointnet_max_radius) > 1e-6:
            raise ValueError(
                f"PointNet max_radius mismatch in {pointnet_file}: "
                f"got {max_radius}, expected {self.pointnet_max_radius}"
            )

        cell_ids = data["cell_ids"].long().reshape(-1)
        features = data["features"]
        if features.ndim != 2 or tuple(features.shape) != (
            cell_ids.numel(), self.pointnet_feature_dim
        ):
            raise ValueError(
                f"Invalid PointNet feature shape in {pointnet_file}: "
                f"cell_ids={tuple(cell_ids.shape)}, features={tuple(features.shape)}"
            )
        if cell_ids.numel() and (
            int(cell_ids.min()) < 0 or int(cell_ids.max()) >= self.height * self.width
        ):
            raise ValueError(f"Out-of-range cell_ids in {pointnet_file}")

        features = features.float()
        if not torch.isfinite(features).all():
            raise ValueError(f"NaN or Inf found in {pointnet_file}")
        ...
        occupied = torch.zeros(self.height * self.width, dtype=torch.bool)
        dense[:, cell_ids] = features.transpose(0, 1)
        occupied[cell_ids] = True
        return (
            dense.reshape(self.pointnet_feature_dim, self.height, self.width),
            occupied.reshape(self.height, self.width),
        )

    def align_pointnet_mask(self, bev_file, pointnet_tensor, occupied, mask_t):
        polar_occupied = mask_t.squeeze(0) > 0
        different = int(torch.count_nonzero(occupied != polar_occupied))
        union = int(torch.count_nonzero(occupied | polar_occupied))
        mismatch_ratio = different / max(union, 1)
        if mismatch_ratio > 0.01:
            raise ValueError(
                f"PointNet/Polar occupancy mismatch for {bev_file}: "
                f"{different}/{union} different occupied cells "
                f"({mismatch_ratio:.2%})"
            )
        return pointnet_tensor * polar_occupied.unsqueeze(0)
    ...
    def apply_bev_copy_paste(self, proj_tensor, pointnet_tensor, mask_t, labels_t,
                             prev_labels_t=None, next_labels_t=None):
        paste_mask_t = torch.zeros_like(mask_t).float()

        if len(self.bev_files) == 0:
            return proj_tensor, pointnet_tensor, mask_t, labels_t, paste_mask_t
        ...
        for attempt in range(12):
            ...
            if obj_cls == 7:
                ...
                vehicle_mask = patch_label == 2

                # bicycle がほとんど無いなら不自然なのでスキップ
            ...
            patch_obj_mask = paste_mask.unsqueeze(0).float()
            ...
            src_pointnet, src_pointnet_occupied = self.load_pointnet_feature(src_file)
            src_pointnet = self.align_pointnet_mask(
                src_file, src_pointnet, src_pointnet_occupied, src_mask
            )
            patch_pointnet = src_pointnet[:, y1:y2, x1:x2]
            ...
            pointnet_tensor[:, ty:ty+h, tx:tx+w] = torch.where(
                m.expand_as(patch_pointnet),
                patch_pointnet,
                pointnet_tensor[:, ty:ty+h, tx:tx+w]
            )
    ...
    def __getitem__(self, index):
        ...
        pointnet_tensor, pointnet_occupied = self.load_pointnet_feature(pt_file)
        pointnet_tensor = self.align_pointnet_mask(
            pt_file, pointnet_tensor, pointnet_occupied, mask_t
        )
        ...
        if do_copy_paste:
            ...
            for _ in range(num_paste):
                (proj_tensor_new, pointnet_tensor_new, mask_t_new,
                 labels_t_new, paste_mask_new) = self.apply_bev_copy_paste(
                    proj_tensor, pointnet_tensor, mask_t, labels_t,
                    prev_labels_t=prev_labels_t, next_labels_t=next_labels_t
                )

                if paste_mask_new.sum() > 0:
                    proj_tensor = proj_tensor_new
                    pointnet_tensor = pointnet_tensor_new
                    mask_t = mask_t_new
        ...
        if self.is_train:
            ...
            if roll_shift > 0:
                ...
                pointnet_tensor = torch.roll(pointnet_tensor, shifts=roll_shift, dims=2)
            ...
            if torch.rand(1) > 0.5:
                ...
                pointnet_tensor = torch.flip(pointnet_tensor, dims=[2])
            ...
            if torch.rand(1) > 0.5:
                ...
                pointnet_tensor = pointnet_tensor * drop_mask
        ...
        hybrid_tensor = torch.cat([proj_tensor, pointnet_tensor], dim=0)
        expected_channels = 7 + self.pointnet_feature_dim
        if hybrid_tensor.shape[0] != expected_channels:
            raise RuntimeError(
                f"Hybrid feature has {hybrid_tensor.shape[0]} channels, "
                f"expected {expected_channels}"
            )

        return (
            hybrid_tensor, mask_t, labels_t, dummy_list, path_seq, path_name,
