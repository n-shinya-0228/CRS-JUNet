...
class SJNet3_PointNet(nn.Module):
    """
    Lighter defaults:
      - aspp_out: 384 -> 256
    ...
    def __init__(
        ...
        self.feature_channels = self.in_channels - 1

        if self.in_channels != 24:
            raise ValueError(
                f"SJNet3_PointNet expects 24 channels "
                f"(7 handcrafted + 16 PointNet + 1 mask), got {self.in_channels}"
            )

        assert aspp_out % swa_heads == 0, f"aspp_out({aspp_out}) must be divisible by swa_heads({swa_heads})"

        self.stem_feat = conv_bn_act(self.feature_channels, base_ch, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        feat_in = x[:, :self.feature_channels, :, :]
        m = x[:, self.feature_channels:self.in_channels, :, :]
