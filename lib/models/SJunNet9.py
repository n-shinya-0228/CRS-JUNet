...
class SJunNet9(nn.Module):
    ...
    def __init__(
        ...
        # ★ 入力特徴量を 4 から 5 に変更！ (max_z, mean_z, max_r, density, z_diff)
    ...
    def forward(self, x):
        # ★ x は全部で6ch (インデックス0〜5) になります。
        feat_in = x[:, :5, :, :]  # インデックス0〜4（5つの特徴量）を取得
        m = x[:, 5:6, :, :]       # インデックス5（本物のマスク）を取得

        s0 = self.stem_feat(feat_in)
        # ★ ここで本物のマスクが掛かり、点のない真空地帯が完全にゼロになる！
        s0 = s0 * m
