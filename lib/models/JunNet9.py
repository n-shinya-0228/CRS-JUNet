# -*- coding: utf-8 -*-
# ChatNet4 (Mask-Aware Stem + ICM + CAM + MCSPN)
...
# ---- ICM: Inception-like Context Module (Lite-HDSeg inspired) ----
class ICM(nn.Module):
    """ Multi-scale context extractor (lightweight).
        branches: 3x3 (d=1), 5x5 (depthwise5x5), 3x3 (d=2) + image-level pooling
    """
    def __init__(self, in_ch=5, c1=24, c2=32, c3=48):
        super().__init__()
        self.b1 = conv_bn_act(in_ch, c1, 3, 1, 1, d=1)                 # local
        self.b2 = nn.Sequential(                                        # larger kernel
            nn.Conv2d(in_ch, c1, 5, 1, 2, bias=False, groups=1),
            nn.BatchNorm2d(c1),
            nn.ReLU(inplace=True)
        )
        self.b3 = conv_bn_act(in_ch, c1, 3, 1, 2, d=2)                  # dilated
        self.mix1 = conv_bn_act(c1*3, c2, 1, 1, 0)                      # fuse
    ...
    def forward(self, x):
        ...
        h = self.mix1(h)
        h = torch.cat([self.ms1a(h), self.ms1b(h), F.interpolate(self.img(x), size=x.shape[-2:], mode='bilinear', align_corners=False)], dim=1)
        h = self.mix2(h)
        return h
...
# ---- CAM: Context Aggregation Module (SqueezeSegV2/Lite-HDSeg系) ----
class CAM(nn.Module):
...
# ---- MCSPN: Multi-class Spatial Propagation Network (Lite-HDSeg inspired) ----
class MCSPN(nn.Module):
    ...
    def forward(self, feats, logits):
        ...
        # ---- make sure feats has same spatial size as logits ----
        if feats.shape[-2:] != (H, W):
            feats = F.interpolate(feats, size=(H, W), mode='bilinear', align_corners=False)

        # ---- 4-direction guidance weights (per-class) ----
        g = self.guidance(feats).view(B, K, 4, H, W)  # [left, right, up, down]
        g = torch.softmax(g, dim=2)

        h = logits

        # helpers: keep tensor size strictly (H,W)
        def shift_left(x):   # neighbor from j+1 -> shift content left
            # pad right by 1, then drop first col
            return F.pad(x, (0, 1, 0, 0))[:, :, :, 1:]  # (B,K,H,W)
        def shift_right(x):  # neighbor from j-1 -> shift content right
            return F.pad(x, (1, 0, 0, 0))[:, :, :, :W]
        def shift_up(x):     # neighbor from i+1 -> shift content up
            return F.pad(x, (0, 0, 0, 1))[:, :, 1:, :]
        def shift_down(x):   # neighbor from i-1 -> shift content down
            return F.pad(x, (0, 0, 1, 0))[:, :, :H, :]

        for _ in range(self.T):
            left  = shift_right(h)  # neighbor at (i,j-1)
            right = shift_left(h)   # neighbor at (i,j+1)
            up    = shift_down(h)   # neighbor at (i-1,j)
            down  = shift_up(h)     # neighbor at (i+1,j)

            # g[:, :, 0]=left, 1=right, 2=up, 3=down
            agg = (g[:, :, 0] * left +
                   g[:, :, 1] * right +
                   g[:, :, 2] * up +
                   g[:, :, 3] * down)

            # self weight so that weights sum to 1
            self_w = 1.0 - g.sum(dim=2)  # (B,K,H,W)
            h = self_w * h + agg
...
class JunNet9(nn.Module):
    ...
    def __init__(self, in_channels: int = 6, nclasses: int = 20, drop: float = 0.2,
                 ...
                 return_aux: bool = True, spn_steps:int = 4):
        super().__init__()
        ...
        self.icm = ICM(in_ch=5, c1=24, c2=32, c3=base_ch)   # ICM on (range,xyz,rem)
        ...
        # MCSPN (refinement on per-class logits guided by features)
        self.mcspn = MCSPN(ch_in=base_ch, nclasses=nclasses, T=spn_steps)

    def forward(self, x):
        # x: (B,6,H,W)
        feat_in = x[:, :5, :, :]          # (range, x, y, z, rem)
        m = x[:, 5:6, :, :]

        s0 = self.icm(feat_in)            # ICM
        g  = torch.tanh(self.stem_mask(m))
        s0 = s0 * (1.0 + g)               # mask-aware gate
        ...
        logits_raw = self.final_logits(feat)
        # MCSPN refinement (Lite-HDSeg: class-wise propagation)
        logits_refined = self.mcspn(feat, logits_raw)

        return {"logits": logits_refined, "logits_raw": logits_raw, "aux2": aux2, "aux4": aux4, "boundary": boundary}
