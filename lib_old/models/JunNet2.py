...
class JunNet2(nn.Module):
    def __init__(self, in_channels, nclasses, drop=0, use_mps=True):
        ...
        self.resBlock1 = BasicBlock(512, 512, drop)
        self.downBlock1 = Downblock(512, 512, drop)


        self.transformer = VisionTransformer(img_size_h=64, img_size_w=256, patch_size=8, num_inputlayer_units=512, num_heads=4, num_mlp_units=512, num_layers=3) #出力(bs, 256+1, 512)
        ...
        self.dec_block1 = Upblock(128 + 512, 256) # -> 64x64, 64

        self.upconv2 = nn.Sequential(
            nn.Upsample(scale_factor=(4, 16), mode='bilinear', align_corners=False),
            nn.Conv2d(256, 128, kernel_size=3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
        self.dec_block2 = Upblock(128 + 32, 64) # -> 64x64, 64
    ...
    def forward(self, x):
        ...
        down1 = self.pool(downCntx)
        downCntx2 = self.first_blocks_sequential2(down1)        #downCntx = (32, 64, 256)

        down2 = self.transformer(downCntx2)
        vit_spatial = down2[:, 1:, :].transpose(1, 2).reshape(   #vit_spatial = (512, 16, 16)
            down2.size(0), 512, 16, 16
        )

        skip0 = self.resBlock1(vit_spatial)           #skip0 = (512, 16, 16)
        down3 = self.downBlock1(skip0)             #down3 = (512, 8, 8)  
        


        dec0_out = self.upconv1(down3)      #skip0 = (128, 16, 16)
        dec1_concat = torch.cat([dec0_out, skip0], dim=1) # -> (B, 128+512, 64, 64)
        dec1_out = self.dec_block1(dec1_concat) # -> (B, 256, 64, 64)
        
        dec2_out = self.upconv2(dec1_out)       #dec2_out = (128, 64, 256)
        dec3_concat = torch.cat([dec2_out, downCntx2], dim=1) # -> (B, 128+32, 64, 256)
        dec3_out = self.dec_block2(dec3_concat) # -> (B, 64, 64, 256)
